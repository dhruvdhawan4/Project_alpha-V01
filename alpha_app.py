...
import io
import math  # FIXED: Explicitly imported to prevent calculation engine crashes
import sqlite3
import datetime
import requests
import numpy as np
...
# FIXED: Universal fixed-offset timezone delta used to completely eliminate Linux/ZoneInfo server deployment bugs
...
class QuantEngine:
    ...
    def generate_picks(target_date, universe, total_cap):
        ...
        # FIXED: Decoupled text and bar elements to maximize browser execution compatibility across versions
        ...
        # Isolate top 3 predictive vectors based on raw ML confidence thresholds
        picks = sorted(picks, key=lambda x: x["confidence"], reverse=True)[:3]
        
        # Capital Allocation: Rounded UP dynamically to the nearest 100 INR
        alloc_per_stock = 0
        if len(picks) > 0:
