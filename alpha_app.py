...
def fetch_indices():
    ...
    for name, ticker in indices.items():
        try:
            # FIXED: Expanded to 5d to ensure we always capture the last 2 trading days even on weekends
...
def analyze_market_regime():
    try:
        ...
        # FIXED: Ensure we have enough data points so the math doesn't crash on weekends
    ...
    if clock_state["is_live"]:
        ...
        if active_picks:
            ...
            for row in active_picks:
                # FIXED: Skip broken DB rows entirely so they don't crash the loop
        ...
        if not next_data:
            ...
            next_rows = []
            for r in next_data:
                # FIXED: Prevent string format crashes by checking for 'is not None' on EVERY attribute
                next_rows.append({
                    "Stock": r["ticker"], 
                    "Call": r["direction"], 
