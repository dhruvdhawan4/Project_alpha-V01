...
class QuantEngine:
    ...
    def analyze_asset(ticker):
        try:
            # FIXED: Increased to 3mo to guarantee rolling(20) never fails due to holidays
            ...
            if abs(total_score) < 0.2: return None
    ...
    def generate_picks(target_date, universe, total_cap):
        ...
        try:
            ...
            # FIXED: Massive fallback list to ensure picks are found even if NSE blocks download
            tickers = [
                "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "ICICIBANK.NS", "BHARTIARTL.NS",
                "SBIN.NS", "INFY.NS", "ITC.NS", "LT.NS", "BAJFINANCE.NS", "HCLTECH.NS", 
                "MARUTI.NS", "SUNPHARMA.NS", "ADANIENT.NS", "KOTAKBANK.NS", "TITAN.NS", 
                "ONGC.NS", "TATAMOTORS.NS", "NTPC.NS", "AXISBANK.NS"
            ]
...
if st.session_state.page == "home":
    ...
    # FIXED: Re-structured layout to prevent truncating numbers and to show parameters instantly
    col1, col2 = st.columns([1.5, 1])
    
    with col1:
        if regime_data:
            st.metric(label="Overall Market Scenario", value=regime_data["regime"])
            # FIXED: Removed expander. Parameters are now directly visible.
            st.markdown(f"""
            <div style='background-color: #161b22; padding: 10px; border-radius: 5px; border: 1px solid #30363d; font-size: 0.9em; margin-top: -10px;'>
            <b>Parameters:</b>&nbsp; 
            20-EMA: {'Bullish 🟢' if regime_data['current'] > regime_data['ema20'] else 'Bearish 🔴'} &nbsp;|&nbsp; 
            50-SMA: {'Bullish 🟢' if regime_data['current'] > regime_data['sma50'] else 'Bearish 🔴'} &nbsp;|&nbsp; 
            RSI(14): {regime_data['rsi']:.1f} &nbsp;|&nbsp; 
            MACD: {'Bullish 🟢' if regime_data['macd'] > 0 else 'Bearish 🔴'}
            </div>
            """, unsafe_allow_html=True)
    
    with col2:
        # FIXED: Will now dynamically pull ONLY the exact universe you selected in the sidebar
        selected_index_name = universe_selection.replace("_", " ")
        d = indices_data.get(selected_index_name)
        if d: 
            st.metric(selected_index_name, f"{d['current']:,.2f}", f"{d['change']:+.2f} ({d['pct']:+.2f}%)")
        else: 
            st.metric(selected_index_name, "Data Unavailable", "N/A")
    ...
    if clock_state["is_live"]:
        ...
        if active_picks:
            ...
            for row in active_picks:
                if row["target_entry"] is None or row["allocation"] is None: continue
                try:
        ...
        if not next_data:
            ...
            for r in next_data:
                next_rows.append({
                    "Stock": r["ticker"], 
                    "Call": r["direction"], 
                    "Last Close Price": f"Rs. {r['current_price']:,.2f}" if r['current_price'] is not None else "N/A", 
                    "Entry Price": f"Rs. {r['target_entry']:,.2f}" if r['target_entry'] is not None else "N/A", 
                    "Exit Price": f"Rs. {r['target_exit']:,.2f}" if r['target_exit'] is not None else "N/A", 
                    "Confidence": f"{r['confidence']}%" if r['confidence'] is not None else "N/A", 
                    "Allocation": f"Rs. {r['allocation']:,.0f}" if r['allocation'] is not None else "N/A", 
                    "Expected Returns": f"+{r['expected_return']:.1f}%" if r['expected_return'] is not None else "N/A"
                })
